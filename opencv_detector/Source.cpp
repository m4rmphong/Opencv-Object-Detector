...
"{ cfg            | data/yolo.cfg      | model configuration }"
"{ model          | data/yolo.weights      | model weights       }"
"{ camera_device  | 1     | camera device number}"
"{ source         | data/dog.jpg      | video or image for detection}"
"{ style          | box   | box or line style draw }"
"{ min_confidence | 0.24  | min confidence      }"
"{ class_names    | data/coco.names      | File with class names, [PATH-TO-DARKNET]/data/coco.names }";
...
int main(int argc, char** argv)
{
	...
	else
	{
		...
		if (!cap.isOpened())
		{
			cout << "Couldn't open image or video: " << parser.get<String>("source") << endl;
			return -1;
		}
	}
	...
	for (;;)
	{
		...
		for (int i = 0; i < detectionMat.rows; i++)
		{
			...
			if (confidence > confidenceThreshold)
			{
				...
				int baseLine = 0;
				Size labelSize = getTextSize(label, FONT_HERSHEY_SIMPLEX, 0.5, 3, &baseLine);
